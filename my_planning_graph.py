...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        intersect_add_rem = set(node_a1.action.effect_add).intersection(set(node_a2.action.effect_rem))
        interset_rem_add = set(node_a2.action.effect_add).intersection(set(node_a1.action.effect_rem))
        return bool(intersect_add_rem) or bool(interset_rem_add)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        interfere_a_b_pos = bool(set(node_a1.action.effect_add).intersection(set(node_a2.action.precond_neg)))
        interfere_a_b_neg = bool(set(node_a1.action.effect_rem).intersection(set(node_a2.action.precond_pos)))
        interfere_b_a_pos = bool(set(node_a2.action.effect_add).intersection(set(node_a1.action.precond_neg)))
        interfere_b_a_neg = bool(set(node_a2.action.effect_rem).intersection(set(node_a1.action.precond_pos)))
        return interfere_a_b_pos or interfere_a_b_neg or interfere_b_a_pos or interfere_b_a_neg
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        def is_precond_mutex(a, b):
            for precondition_state in a:
                if precondition_state.mutex.intersection(b):
                    return True
            return False

        return is_precond_mutex(node_a1.parents, node_a2.parents) or is_precond_mutex(node_a2.parents, node_a1.parents)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        desired_states = {node_s1.symbol, node_s2.symbol}
        for an1 in node_s1.parents:
            an1_effect = set(an1.action.effect_add)
            if desired_states.issubset(an1_effect):
                return False
            for an2 in node_s2.parents:
                an2_effect = set(an2.action.effect_add)
                if desired_states.issubset(an2_effect):
                    return False
                elif not an1.is_mutex(an2):
                    effect = an1_effect.union(an2_effect)
                    if desired_states.issubset(effect):
                        return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        if isinstance(self.problem.goal, list):
            goals = [g for g in self.problem.goal]
        else:
            goals = [self.problem.goal]

        def level_cost(goal):
            for level,s_nodes_set in enumerate(self.s_levels):
                level_literals = set(n.symbol for n in s_nodes_set if n.is_pos)
                if goal in level_literals:
                    # print("goal {} first found on level {}".format(goal,level))
                    return level
            return -1

        for goal in goals:
            cost = level_cost(goal)
            if cost < 0:
                raise Exception("Solution does not exist, goal {} cannot be achieved".format(goal))
            level_sum += cost
